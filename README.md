...
Stripe has two completely separate sets of keys. The Dashboard has a **Test mode / Live mode toggle** in the top-right corner — make sure it matches what you intend.

|                         | Test (dev/local)                                          | Live (production)                                                      |
| ----------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------- |
| `STRIPE_SECRET_KEY`     | `sk_test_…`                                               | `sk_live_…`                                                            |
| `STRIPE_CLIENT_ID`      | `ca_…` from Connect settings **with test toggle ON**      | `ca_…` from Connect settings **with live toggle ON** — different value |
| `STRIPE_WEBHOOK_SECRET` | generated by `stripe listen` locally — not from Dashboard | `whsec_…` from the registered endpoint in Dashboard                    |

**Rules:**

- `sk_live_…` keys go **only** in GCP Secret Manager for production — never in `.env` or dev environments.
- The `STRIPE_CLIENT_ID` is different for test vs live — make sure the Dashboard toggle is set to the right mode when you copy it.
