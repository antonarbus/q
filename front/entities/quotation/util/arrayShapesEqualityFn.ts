...
export const arrayShapesEqualityFn: EqualityFn = (prev: Obj[], current: Obj[]): boolean => {
  ...
  const currentSliced = current.slice(0, BOOKMARK_POS_AT_BLOCKS - 1).filter(Boolean)
  ...
  const idsDoNotMatch = prevSliced.some(
    (_item: Obj, index: number) => prevSliced[index]?.id !== currentSliced[index]?.id,
  )
