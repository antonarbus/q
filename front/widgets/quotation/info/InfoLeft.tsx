...
export const InfoLeft = (): React.ReactNode => {
  ...
  const resolveSharedLabel = (): string => {
    if (permissionLevel === 'PUBLIC') { return 'Public' }
    if (permissionLevel === 'SHARED') { return 'Shared' }
    if (accessLevel === 'everyone') { return 'Public' }
    return 'Shared'
  }
  ...
  const resolveBadge = (): Badge | null => {
    if (permissionLevel === 'NEW') { return { color: 'text.disabled', label: 'New' } }
  ...
  return (
    <Box sx={{ width: '100px', maxWidth: '100px', height: '18px', display: 'flex', alignItems: 'center' }}>
      {badge !== null && (
